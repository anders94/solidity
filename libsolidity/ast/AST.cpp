...
shared_ptr<FunctionType const> FunctionDefinition::functionType(bool _external) const
{
	if (_external)
	{
		switch (visibility())
...
shared_ptr<FunctionType const> VariableDeclaration::functionType(bool _external) const
{
	if (!_external)
		return {};
	switch (visibility())
